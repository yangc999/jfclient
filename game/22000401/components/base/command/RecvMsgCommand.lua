...
local RecvMsgCommand = class("PenetrateMsgCommand", SimpleCommand)

function RecvMsgCommand:execute(notification)
    print("-------------->RecvMsgCommand:execute")
    ...
    print("-------------->RecvMsgCommand:tp = " .. tostring(tp))
    if name == GameConstants.PENETRATE_MSG then
        ...
        elseif tp == 3 then  -- 3为游戏消息
            ...
            if GameUtils:getInstance():getGameType() == 3 and player.CurPlayerNums ~= MyGameConstants.PLAYER_COUNT then
                return
            end
            ...
            if gameTP == 14 then  -- 断线重连
                print("game station")
                --gameFacade:sendNotification(MyGameConstants.C_CLOSE_LOADING_ANIMATE) -- 关闭加载动画界面     
