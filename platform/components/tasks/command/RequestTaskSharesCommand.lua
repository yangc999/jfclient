...
local RequestTaskSharesCommand = class("RequestTaskSharesCommand", SimpleCommand)

function RequestTaskSharesCommand:execute(notification)
    ...
    print("RequestTaskSharesCommand:execute")
	...
	local function onReadyStateChange()
		if xhr.readyState == 4 and xhr.status >= 200 and xhr.status < 207 then
            ...
            if res3.iRetCode == 0 then
                taskproxy:getData().taskInfo = res3.vTaskList
                dump(taskproxy:getData().taskInfo,"taskProxy.taskInfo")            
            end
		else
	...
	local pak1 = {
		...
		vTaskidList = taskproxy:getData().taskID,        --指定任务ID
		sTriggerKey = 0       --触发参数
	}
...
return RequestTaskSharesCommand
